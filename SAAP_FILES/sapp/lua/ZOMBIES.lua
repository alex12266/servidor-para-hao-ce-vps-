...
-- Kill messages
infect_msg = "infectado"
suicide_msg = "no podia soportar la presion!"
unknown_msg = "fue infectado debido a una fuerza misteriosa."

-- Other messages
human_again_msg = "se ha vuelto humano nuevamente por matar %s humanos!"
game_over_msg = "Todos los humanos han sido infectados! El juego ha terminado..."
no_more_zombies_msg = "No hay mas zombies!"
no_zombies_countdown_msg = "Un jugador aleatorio se convertira en zombie"
lastman_msg = "es la ultima persona viva, encuentrala y comete su cerebro!!!"
...
function OnPlayerJoin(PlayerIndex)
	...
	if game_active ~= true then
		...
		if players > 1 then
			...
			say(PlayerIndex, "Espera a que se unan mas jugadores.")
		end
	else
		set_navs()
		if players > 1 then
			...
			game_active = false
			say(PlayerIndex, "El juego se ha detenido, no hay suficientes jugadores.")
		end
	end
end
...
function start_game_timer()
	...
	count = count + 1
	msg = "El juego comenzara en " .. 6 - count
	if count >= 6 then
